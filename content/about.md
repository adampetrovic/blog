+++
title = "About"
menu = "main"
weight = 20
+++

I'm Adam Petrovic, a senior engineer at Atlassian based in Sydney, Australia.

I build and operate distributed systems: storage platforms, metrics pipelines, reliability tooling, and the connective tissue that lets teams understand what their software is doing in production. Lately that has meant working on Atlassian's internal S3-compatible storage service, spanning Kubernetes, AWS/GCP, data residency, sharding, backup and restore, cost attribution, and the awkward-but-important details of proving data is where it should be.

Before that, I spent several years in observability, including high-volume metrics infrastructure, synthetic monitoring, and large migration work. I have a soft spot for time series databases and telemetry systems: the ingestion paths, cardinality traps, percentile maths, retention trade-offs, and operational habits that turn a pile of datapoints into something engineers can trust during an incident.

Outside work I run a [home-ops](https://github.com/adampetrovic/home-ops) environment that is less a "smart home" project and more a small production system with nobody on call but me. It is a bare-metal Kubernetes cluster managed with Talos Linux, Flux, Helm, Kustomize, SOPS, Rook-Ceph, VolSync, Home Assistant, and a growing collection of automations. It is where I test ideas about boring infrastructure, recovery, observability, and how much ceremony is actually useful at home scale.

You can also [view my resume](/resume.pdf) or find me on [LinkedIn](https://www.linkedin.com/in/adampetrovic/).
